...
|---|---|---|
| **Local MySQL (default)** | `STORAGE_BACKEND=mysql` and `DATABASE_URL` | Tables via [Prisma ORM](prisma/schema.prisma); browse with `npm run db:studio` |
| **File (fallback / opt-in)** | `STORAGE_BACKEND=file`, or mysql requested without `DATABASE_URL` | `data/runtime/runtime-store.json` |
