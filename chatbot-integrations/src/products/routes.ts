...
function parseSearchBody(body: unknown): ProductSearchQuery {
  ...
  return {
    ...
    uvProtection: pickString(data.uv_protection) ?? pickString(data.uvProtection),
    polarized: pickString(data.polarized),
    lensColor: pickString(data.lens_color) ?? pickString(data.lensColor),
    lensType: pickString(data.lens_type) ?? pickString(data.lensType),
    lensFeature: pickString(data.lens_feature) ?? pickString(data.lensFeature),
    lensDuration: pickString(data.lens_duration) ?? pickString(data.lensDuration),
    multifocal: pickBool(data.multifocal) ?? pickString(data.multifocal),
